...
LDFLAGS = -s -w \
  ...
  -X github.com/vulnetix/cli/cmd.commit=$(COMMIT) \
  -X github.com/vulnetix/cli/cmd.buildDate=$(BUILD_DATE)
...
build-release: clean
	...
	@GOOS=linux GOARCH=arm64 go build -ldflags "$(LDFLAGS)" -o $(OUTPUT_DIR)/$(BINARY_NAME)-linux-arm64 .
	@echo "Building for Linux ARM..."
	@GOOS=linux GOARCH=arm go build -ldflags "$(LDFLAGS)" -o $(OUTPUT_DIR)/$(BINARY_NAME)-linux-arm .
	@echo "Building for Linux 386..."
	@GOOS=linux GOARCH=386 go build -ldflags "$(LDFLAGS)" -o $(OUTPUT_DIR)/$(BINARY_NAME)-linux-386 .
	...
	@GOOS=windows GOARCH=arm64 go build -ldflags "$(LDFLAGS)" -o $(OUTPUT_DIR)/$(BINARY_NAME)-windows-arm64.exe .
	@echo "Building for Windows ARM..."
	@GOOS=windows GOARCH=arm go build -ldflags "$(LDFLAGS)" -o $(OUTPUT_DIR)/$(BINARY_NAME)-windows-arm.exe .
	@echo "Building for Windows 386..."
	@GOOS=windows GOARCH=386 go build -ldflags "$(LDFLAGS)" -o $(OUTPUT_DIR)/$(BINARY_NAME)-windows-386.exe .
...
dev:
	...
	@mkdir -p $(OUTPUT_DIR)
	go build -ldflags "-X github.com/vulnetix/cli/cmd.version=$(VERSION)-dev -X github.com/vulnetix/cli/cmd.commit=$(COMMIT) -X github.com/vulnetix/cli/cmd.buildDate=$(BUILD_DATE)" -o $(OUTPUT_DIR)/$(BINARY_NAME) .
