---
title: "VNX-SEC-057 – Grafana API / Service Account Token"
description: "Detects Grafana Cloud API tokens and Grafana service-account tokens hardcoded in source code."
---

## Overview

This rule detects grafana api / service account token matching the `glc_..., glsa_...` pattern hardcoded anywhere in source files. Detects Grafana Cloud API tokens and Grafana service-account tokens hardcoded in source code.

**Severity:** Critical | **CWE:** [CWE-798 – Use of Hard-coded Credentials](https://cwe.mitre.org/data/definitions/798.html)

## Why This Matters

Hardcoded grafana api / service account token values are routinely scraped by automated bots within minutes of a public push, and then used to access the account, exfiltrate data, or pivot into downstream services. Once the credential is in git history it is permanently in third-party hands.
