---
title: "VNX-SEC-080 – GitHub OAuth / App / Refresh Token"
description: "Detects GitHub OAuth, App user-to-server, App server-to-server, and refresh tokens hardcoded in source code."
---

## Overview

This rule detects github oauth / app / refresh token matching the `gho_, ghu_, ghs_, ghr_` pattern hardcoded anywhere in source files. Detects GitHub OAuth, App user-to-server, App server-to-server, and refresh tokens hardcoded in source code.

**Severity:** Critical | **CWE:** [CWE-798 – Use of Hard-coded Credentials](https://cwe.mitre.org/data/definitions/798.html)

## Why This Matters

Hardcoded github oauth / app / refresh token values are routinely scraped by automated bots within minutes of a public push, and then used to access the account, exfiltrate data, or pivot into downstream services. Once the credential is in git history it is permanently in third-party hands.
