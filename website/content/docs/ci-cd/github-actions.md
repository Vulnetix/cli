...
**Versions in this page are current as of writing** (`Vulnetix/cli@v3.55.2`).
...
|----------|-------------|
| **Native action** — `uses: Vulnetix/cli@v3.55.2` | Quickstart, `upload`, and `gha` artifact collection. Requires `actions/setup-go`. |
...
```yaml
      - name: Install Vulnetix CLI
        run: curl -fsSL https://cli.vulnetix.com/install.sh | sh -s -- --version v3.55.2
```
...
**Action reference not found** — there is no moving `v1`/`v2`/`v3` tag. Pin an exact release (`Vulnetix/cli@v3.55.2`) or, for unreleased changes, `Vulnetix/cli@main`.
...
Everything above was verified against `Vulnetix/cli` `v3.55.2`. Flags, default output paths, and action inputs change between releases. If a snippet no longer works, [open an issue](https://github.com/Vulnetix/cli/issues/new) with the release you are on and we will correct the documentation.
