  ...
  default:
    - step:
        ...
        script:
          ...
          - vulnetix --org-id "$VULNETIX_ORG_ID" --task release --project-name "$BITBUCKET_REPO_SLUG" --team-name "$VULNETIX_TEAM_NAME"
```
...
```yaml
# bitbucket-pipelines.yml - Parallel security assessments
pipelines:
  default:
    - step:
        name: Setup
...
pipelines:
  default:
    - step:
        name: Security Assessment (Corporate Network)
        script:
...
pipelines:
  default:
    - step:
        name: Optimized Security Assessment
...
pipelines:
  default:
    - step:
        name: Security Assessment (Custom CA)
        script:
...
pipelines:
  default:
    - step:
        name: Memory-Optimized Assessment
...
pipelines:
  default:
    - step:
        name: Cached Security Assessment
        script:
          - export PATH=$PATH:$HOME/.local/bin
          - |
...
vulnetix --org-id "$VULNETIX_ORG_ID" --task release --project-name "test-project"
```
