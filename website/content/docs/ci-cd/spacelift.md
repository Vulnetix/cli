...
title: "Spacelift"
weight: 26
description: "Integrate Vulnetix CLI into Spacelift for infrastructure security scanning."
---
...
Credentials come from Spacelift environment variables, marked as secret so they are write-only.
...
`.spacelift/vulnetix.sh`:

```bash
#!/bin/bash
# Spacelift before_init hook
...
{{< callout type="info" >}}
Spacelift is IaC-shaped, so `vulnetix iac` is usually the command you want, not the full `scan`.
{{< /callout >}}
...
Attach to the run as a log, or push to your own store.
...
To surface a breach without blocking the merge: Run the scan in `before_apply` and tolerate a non-zero exit explicitly.
