...
title: "Earthly"
weight: 29
description: "Integrate Vulnetix CLI into Earthly build targets for reproducible vulnerability scanning."
---

Security scanning with Earthly builds.
...
Credentials come from `--secret` on `RUN`, which mounts the value for that command only and never records it in a layer.
...
vulnetix-scan:
    ...
    COPY . /workspace
    WORKDIR /workspace
    RUN --secret VULNETIX_ORG_ID --secret VULNETIX_API_KEY vulnetix auth verify
    RUN --secret VULNETIX_ORG_ID --secret VULNETIX_API_KEY vulnetix scan --severity high
```
...
{{< callout type="info" >}}
`RUN --secret VAR` is the only correct way to pass a credential. A `--build-arg` is recorded in image metadata.
{{< /callout >}}
...
To surface a breach without blocking the merge: `RUN ... || true` inside the target.
