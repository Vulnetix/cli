...
title: "Buildkite"
weight: 12
description: "Integrate Vulnetix CLI into Buildkite pipelines for automated vulnerability scanning."
---

Security scanning in Buildkite pipelines.
...
Credentials come from An agent environment hook, or the `secrets` helper of your Elastic CI stack.
...
{{< callout type="info" >}}
Buildkite agents are long-lived. Do not run `vulnetix auth login` on them; a credentials file written into a shared agent home outlives the build.
{{< /callout >}}
...
To surface a breach without blocking the merge: `soft_fail: true` on the step.
