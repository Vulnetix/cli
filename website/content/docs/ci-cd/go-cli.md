...
title: "Go CLI"
weight: 10
description: "Install Vulnetix CLI via go install and integrate with Go security tools like govulncheck and gosec for comprehensive vulnerability scanning."
---

Install and run Vulnetix CLI directly with Go.
...
Credentials come from your shell environment, or the OS keychain via `vulnetix auth login --store keyring`.
...
vulnetix upload --file reports/sbom.json
```
...
{{< callout type="info" >}}
On a developer machine, prefer `--store keyring` over environment variables. See [Credential Storage](/docs/authentication/storage/).
{{< /callout >}}
...
To surface a breach without blocking the merge: Not applicable — you are the build.
