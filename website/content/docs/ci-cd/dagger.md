...
title: "Dagger"
weight: 30
description: "Integrate Vulnetix CLI into Dagger pipelines for portable, containerized vulnerability scanning."
---
...
Credentials come from `dag.setSecret()` / `withSecretVariable`, which keeps the value out of the cache key.
...
{{< callout type="info" >}}
Passing a credential with `withEnvVariable` puts it in the layer cache key. Use `withSecretVariable`.
{{< /callout >}}
...
To surface a breach without blocking the merge: Catch the non-zero exit in your pipeline code.
