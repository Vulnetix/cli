...
title: "TeamCity"
weight: 20
description: "Integrate Vulnetix CLI into TeamCity build configurations for automated vulnerability scanning."
---

Security scanning in TeamCity builds.
...
Credentials come from A `password` type parameter, which TeamCity masks in the build log.
...
{{< callout type="info" >}}
A parameter typed `text` is printed in the build log. Use `password` for the ApiKey.
{{< /callout >}}
...
An artifact path rule (`dist/** => reports.zip`).
...
To surface a breach without blocking the merge: Configure the build step to not fail the build on a non-zero exit.
