...
title: "Docker"
weight: 7
description: "Run Vulnetix CLI as a Docker container for security scanning. Volume mount your workspace and reports for SARIF, CycloneDX, SPDX upload."
---

Run Vulnetix CLI in Docker containers.
...
Credentials come from `-e VULNETIX_ORG_ID -e VULNETIX_API_KEY` with no value, which forwards the value from the host shell so the secret never appears in `docker inspect`.
...
```bash
# There is no published Vulnetix image; install the CLI into a base image.
docker run --rm \
...
{{< callout type="info" >}}
There is no published Vulnetix image. Install the CLI into a base image at run time, or bake your own image once.
{{< /callout >}}
...
Write into the bind-mounted workspace.
...
To surface a breach without blocking the merge: Check the exit status yourself; the container's exit code is the CLI's.
